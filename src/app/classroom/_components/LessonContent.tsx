...
import Image from 'next/image';
import { Lesson } from '../../../types/course.types';
import { FileText, Link as LinkIcon, Download, CheckCircle, XCircle, ExternalLink, Circle } from 'lucide-react';
import { CourseService } from '../../../services/courseService';
...
const LessonContent: React.FC<LessonContentProps> = ({ 
  ...
  const handleFileAction = (fileUrl: string, fileName: string, fileType: string, action: 'download' | 'view') => {
    try {
      if (action === 'view') {
            ...
            {lesson.resources.map((resource) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  {/* View/Open Button */}
                  <button
                    onClick={() => handleFileAction(resource.fileUrl, resource.title, resource.fileType, 'view')}
                  ...
                  {/* Download Button */}
                  <button
                    onClick={() => handleFileAction(resource.fileUrl, resource.title, resource.fileType, 'download')}
      ...
      {lesson.resources && lesson.resources.some(r => isPDF(r.fileType)) && (
        <div className="mt-8">
          ...
          {lesson.resources
            ...
            .map((resource) => (
              <div key={`pdf-${resource._id}`} className="mb-6 border rounded-lg overflow-hidden">
