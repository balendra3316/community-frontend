...
//           error={!!nameError}
//           helperText={nameError || "You must use your real name (letters only, up to 25 characters)"}
//           sx={{ mb: 3 }}
//         />
...
//             color: snackbarSeverity === 'success' ? themeYellow.text : undefined,
//             '& .MuiAlert-icon': {
//               color: snackbarSeverity === 'success' ? themeYellow.primary : undefined
//             }
//           }}
//         >
//           {snackbarMessage}
...
export default function ProfileEditTab({ user, onUpdateStart, onUpdateEnd }: ProfileEditTabProps) {
  const { updateProfile } = useAuth();
  const [nameInput, setNameInput] = useState('');
  const [nameError, setNameError] = useState<string | null>(null);
  const [avatarFile, setAvatarFile] = useState<File | null>(null);
