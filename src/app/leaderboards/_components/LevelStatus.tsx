// // app/leaderboards/_components/LevelStatus.tsx
// "use client";
...
//   const { user } = useAuth(); // assumes backend adds points, level, leaderboardBadges to /auth/me
...
//             <div className={`inline-flex items-center justify-center rounded-full w-12 h-12 font-semibold ${currentSpec?.color ?? "bg-gray-100 text-gray-800"}`}>
//               {currentSpec?.level ?? 0}
//             </div>
//             <div>
//               <div className="text-sm text-gray-500">Current level</div>
//               <div className="text-lg font-semibold">
//                 {currentSpec ? `Level ${currentSpec.level} – ${currentSpec.name}` : "Level 0 – Not started"}
//               </div>
//             </div>
//           </div>
...
//             <div className="text-xl font-semibold">{numberWithCommas(points)}</div>
//           </div>
//         </div>
...
//             <span>
//               {currentSpec ? `${numberWithCommas(currentSpec.points)} pts` : "0 pts"}
//             </span>
//             <span>
//               {nextSpec ? `${numberWithCommas(nextSpec.points)} pts` : "Max"}
//             </span>
//           </div>
...
//               {nextSpec ? (
//                 <>
//                   Next: <span className={`px-2 py-0.5 rounded ${nextSpec.color}`}>{`Level ${nextSpec.level} – ${nextSpec.name}`}</span>
//                 </>
//               ) : (
...
//             {(user as any)?.leaderboardBadges?.length
//               ? (user as any).leaderboardBadges
//                   .sort((a: any, b: any) => a.level - b.level)
//                   .map((b: any) => {
//                     const spec = LEVELS.find(l => l.level === b.level);
//                     const color = spec?.color ?? "bg-gray-100 text-gray-800";
//                     return (
//                       <span key={b.level} className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-sm ${color}`}>
//                         <span className="font-semibold">L{b.level}</span>
//                         <span className="opacity-90">{b.name}</span>
//                       </span>
//                     );
//                   })
