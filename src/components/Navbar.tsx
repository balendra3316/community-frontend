...
import { fetchUnreadCount, updateUnreadCountFromSocket, resetNotificationsStatus} from "../lib/NotificationSlice";
...
const NavBar = () => {
  ...
  const { unreadCount, countStatus } = useSelector((state: RootState) => state.notifications);
  ...
  // --- EFFECT FOR NOTIFICATIONS (API + SOCKET) ---
  useEffect(() => {
    if (!user) return;

    // 1. Fetch initial count via Redux Thunk (only once)
    if (countStatus === 'idle') {
    ...
    const initializeSocketConnection = () => {
     ...
     const handleUnreadCountUpdate = ({ count }: { count: number }) => {
        if (mounted) {
          ...
          const currentCount = store.getState().notifications.unreadCount;

          
          dispatch(updateUnreadCountFromSocket(count));
  ...
  }, [user, countStatus, dispatch]);


  //(All handler functions: handleProfileMenuOpen, handleProfileMenuClose, etc. remain the same)
   const handleProfileMenuOpen = useCallback(
    (event: MouseEvent<HTMLElement>) => {
      setAnchorEl(event.currentTarget);
      setShowNotifications(false);
