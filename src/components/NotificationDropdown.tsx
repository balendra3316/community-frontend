...
const NotificationItem = memo(
  ({
    ...
    const notificationText = useMemo(() => {
      ...
      // Check if post exists and has a title
...
const NotificationDropdown = ({
  ...
  useEffect(() => {
    ...
    const initializeNotifications = async () => {
      try {
        ...
        if (mounted) {
          // Filter out notifications with missing critical data
          const validNotifications = notifs.filter(notif => 
            notif && notif.sender && notif._id
          );
  ...
  const handleNotificationClick = useCallback(
    async (notification: Notification) => {
      // Check if notification has a valid post before proceeding
      if (!notification.post || !notification.post._id) {
        // If no post associated, just mark as read and return
        if (!notification.read) {
          try {
            setNotifications((prev) =>
              prev.map((n) =>
      ...
      if (notification.read) {
        // If already read and has a post, show post detail
        setSelectedPost(notification.post);
        setShowPostDetail(true);
        return;
      }

      // Mark as read and show post detail
      const previousNotifications = [...notifications];
      const previousUnreadCount = unreadCount;
