...
const PostExtensions = memo(({
  ...
  const [linkUrl, setLinkUrl] = useState('');
  const [linkError, setLinkError] = useState('');
  const [youtubeError, setYoutubeError] = useState(''); // State for YouTube URL errors

  const extractYouTubeVideoId = (url: string) => {
    // This regex is more robust and handles regular, shorts, mobile, and other common YouTube URL formats.
  ...
  const handleYouTubeSubmit = () => {
    if (youtubeUrl) {
      ...
      if (videoId) {
        onYoutubeAdd({
          // CORRECTED: Using the standard and working i.ytimg.com URL for thumbnails.
        ...
        setYoutubeError('Invalid YouTube URL. Please use a valid video or Shorts link.');
      }
    }
  };
