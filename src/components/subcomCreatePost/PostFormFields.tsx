...
import { memo } from 'react';
import { TextField, Typography, IconButton, Button } from '@mui/material';
import { Close as CloseIcon, Add as PlusIcon, Delete as DeleteIcon } from '@mui/icons-material';
...
interface PostFormFieldsProps {
  ...
  onRemovePollOption: (index: number) => void;
  onCancelPoll: () => void;
  // **New prop to notify the parent component about which input field is focused.**
  onInputFocus: (target: 'title' | 'content') => void;
}
...
const PostFormFields = memo(({
  ...
  pollOptions, onTitleChange, onContentChange, onRemoveImage,
  onRemoveYoutube, onPollOptionUpdate, onAddPollOption,
  onRemovePollOption, onCancelPoll, onInputFocus,
    ...
    <div className="p-4">
      ...
      <div className="relative">
        <TextField 
          ...
          // **Add onFocus handler to track when this input is active.**
          onFocus={() => onInputFocus('title')}
          InputProps={{ disableUnderline: true, style: { fontSize: '1.125rem', fontWeight: 500 } }} 
          className="mb-1" 
        />
        {containsBadWords(title) && (
          <Typography color="error" variant="caption" className="ml-1 text-red-500">
